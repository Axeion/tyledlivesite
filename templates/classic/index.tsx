...
export const classicKit: ThemeKit = {
  ...
  section: "mb-12",
  h1: "font-serif text-3xl font-semibold text-[#0b1f3a] mb-4",
  h2: "font-serif text-2xl font-semibold text-[#0b1f3a]",
  h3: "font-serif text-lg font-semibold text-[#0b1f3a]",
  card: "rounded border border-[#c9a227]/40 bg-white p-4 shadow-sm",
  button: "inline-block rounded border border-[#0b1f3a] px-3 py-1 text-sm text-[#0b1f3a] hover:bg-[#0b1f3a] hover:text-white",
  link: "text-[#0b1f3a] underline decoration-[#c9a227] underline-offset-2 hover:text-[#c9a227]",
  muted: "text-neutral-600",
  prose: "rich-text rich-text-serif max-w-none",
  badge: "rounded-full bg-[#c9a227]/20 px-2 py-0.5 text-xs font-medium text-[#7a5c00]",
  calendarCell: "min-h-20 bg-white p-1 align-top",
  calendarCellMuted: "min-h-20 bg-neutral-100 p-1 text-neutral-400",
  calendarEvent: "mb-0.5 truncate rounded bg-[#0b1f3a] px-1 py-0.5 text-[10px] text-white",
};
...
export default function ClassicTemplate({ data, page }: TemplateProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f7f5ef] text-neutral-900" data-template="classic">
      <PreviewBanner data={data} />
      <header className="border-b-4 border-[#c9a227] bg-[#0b1f3a] text-white">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4 px-4 py-6">
          <a href={data.links.home} className="flex items-center gap-4">
            {l.logoUrl ? (
              ...
              <span className="flex h-14 w-14 items-center justify-center rounded-full border-2 border-[#c9a227] font-serif text-xl">
                {l.number}
              </span>
            )}
            <span>
              <span className="block font-serif text-2xl font-semibold">{l.name}</span>
              <span className="block text-sm text-[#c9a227]">
          ...
          <nav aria-label="Main">
            <ul className="flex flex-wrap gap-4 text-sm uppercase tracking-wide">
              {navItems(data).map((n) => (
                <li key={n.href}>
                  <a href={n.href} className="hover:text-[#c9a227]">
                    {n.label}
                  </a>
                </li>
              ))}
        ...
        {page.kind === "home" ? (
          <>
            <section className="mb-12 text-center">
              ...
              <h1 className="font-serif text-4xl font-semibold text-[#0b1f3a]">
                {l.name} No. {l.number}
              </h1>
      ...
      <footer className="border-t border-[#c9a227]/40 bg-[#0b1f3a] py-6 text-center text-sm text-white/80">
